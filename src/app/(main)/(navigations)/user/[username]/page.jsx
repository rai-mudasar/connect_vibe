...
export default async function ProfilePage({ params }) {
  ...
  return (
    <div className="w-full overflow-x-hidden bg-bg">
      <div className="w-full flex flex-col justify-center items-center pb-4 bg-whit">
        <Suspense fallback={<ProfileHeaderSkeleton />}>
          <ProfileHeaderWrapper params={params} isPublicView={isPublicView} />
        </Suspense>
      </div>
      ...
      <div className="w-full flex flex-col justify-center items-center">
        <div className="w-[90%] md:w-[70%] max-h-full flex flex-col md:flex-row gap-5 md:gap-2 mt-5">
          <div className="w-full md:w-[35%]">
            <Suspense fallback={<Loading className={'mt-10'} />}>
              <UserDetailWrapper params={params} />
            </Suspense>
          </div>
          <div className="w-full md:w-[65%]">
            <Suspense fallback={<Loading className={'mt-15'} />}>
              <ProfilePostFeedWrapper params={params} isPublicView={isPublicView} />
            </Suspense>
          </div>
        </div>
      </div>
